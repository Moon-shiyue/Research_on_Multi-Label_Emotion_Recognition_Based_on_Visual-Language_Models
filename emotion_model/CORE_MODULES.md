...
>
> 验证结果：**115/115 项测试全部通过**（详见 `module_verification_report.txt`）
...
CVPR 2021 的 Emotion Circle 面向**标签分布学习（LDL）**：一个样本对应一个情感分布，
经向量叠加得到**一个**复合情感向量。而多标记任务需要输出「有哪些情感」，
两者需求不同，因此采用**双头设计**把二者统一：
...
```
     ...
     ┌───────────────┴────────────────┐
     ↓                                ↓
头1 CircularMultiLabelHead      头2 CompoundEmotionHead
   标签级：每个情感一个表示        样本级：整体复合情感
   「有哪些情感」                  「整体落在环上哪里」
   sigmoid → 8 类置信度            注意力池化 → (p, θ, r)
   损失: 非对称损失                损失: 渐进式环形损失
     └───────────────┬────────────────┘
                     ↓
          CircularConsistencyLoss（双向一致性）
        反向：环形向量 → 反解标签分布 ↔ 多标记预测分布
```

**回答「多个标记能否形成环形」**：多标记不是「多个标记各自成环」，
而是**每个情感是环上的一个基本方向，多个标签通过向量叠加形成一个复合情感位置**。
头2 正是做这件事——用真实标签分布构造目标复合向量并监督，再由一致性损失
把两个头绑在一起（环形为多标记提供结构先验，多标记为环形提供离散约束）。
...
分支2 角度类型:  logits_k = W_θk·Z + b_θk            sigmoid → 多标记置信度
                θ_k = 基本角度 + tanh(·)·π            自由学习（软约束）
分支3 强度回归:  a_k = σ(W_ak·Z + b_ak)              强度 ∈ (0,1)
```

**角度约束的演进（方案C）**：早先实现把角度偏移硬截断在 ±π/8，
导致角度几乎等于先验值、「角度」维度学不到东西。现改为**软约束**：

```
L_angle = λ_prior · (θ_pred - θ_prior)²          λ_prior = 0.1
```

模型可自由学习角度（最大 ±π，用于表达复合情感位置），但偏离先验过远会被惩罚——
既保留表达能力，又避免情感类型失去可辨识性。
